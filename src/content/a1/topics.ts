...
Spelling rule: Dutch keeps vowel sounds consistent between the infinitive and the stem. If removing "-en" would change a long vowel into a short one, double the vowel in the stem ("wonen" -> stem "woon", not "won"). If the stem already ends in "-t", __don't add a second one__ ("praten" -> "hij praat", not "praatt").

One more wrinkle: when "jij"/"je" comes after the verb (in questions or after something fronted to the start of the sentence), __drop the final "-t"__: "Werk jij hier?" not "Werkt jij hier?".`,
    examples: [
...
Questions work the same way: yes/no questions put the verb first, then the subject. Questions with a question word (waar, wat, wanneer...) put the question word first, then the verb, then the subject.`,
  ...
  },
  {
    ...
    explanation: `Dutch has two words for "the": **"de"** and **"het"**. Roughly two-thirds of nouns are "de" words; the rest are "het" words, and __there is no reliable rule for which is which, so each new noun is learned together with its article__.
...
For "them", "hen" and "hun" are both technically correct in different grammatical roles (hen after a preposition or as a direct object, hun as an indirect object), but in casual speech most people just use "ze" for both, __and that is the safest default while you are learning__.`,
    examples: [
  ...
  },
  {
    ...
    explanation: `Prepositions link a noun to the rest of the sentence, and in Dutch they often __don't translate word-for-word from English__, so it's worth learning them inside fixed phrases rather than in isolation.
...
Most possessives don't change at all, no matter what noun follows. __The one exception is "ons/onze"__: ==use "ons" before a singular "het" word, and "onze" everywhere else (before a "de" word, singular or plural, and before any plural noun).==
...
==A useful shortcut: if the English sentence could be rephrased with "no" instead of "not a / not any", Dutch wants "geen".== "I don't have a car" -> "I have no car" -> "Ik heb geen auto." But __"I don't have the car" (a specific, definite car) takes "niet"__: "Ik heb de auto niet."
...
If the stem already ends in -t, __you don't skip doubling it__ (a Dutch syllable never ends in two identical consonants written together as separate morphemes, but -t + -te genuinely does give -tte): "praten" -> stem "praat" -> ik praatte. Likewise "antwoorden" -> stem "antwoord" -> ik antwoordde.

Dutch uses the simple past mainly for something that happened in the past and is now finished and unrelated to the present, for describing a state of affairs during a past event, and after "toen" (when, referring to one specific past moment). __For general "I did X" statements in everyday conversation, Dutch actually prefers the present perfect__ (see the next topic) — the simple past shows up more in writing and storytelling.`,
    examples: [
...
To build the past participle of a regular verb: ge + stem + t/d, using the same t-verb/d-verb split as the simple past (see the previous topic). "Maken" (t-verb, stem "maak") -> gemaakt. "Luisteren" (d-verb, stem "luister") -> geluisterd. Two small exceptions: __a participle never doubles its final letter__ (so "rusten" -> gerust, not "gerustt"), and __verbs starting with the unstressed prefixes be-, er-, ge-, her-, ont-, or ver- skip the ge- entirely__ ("betalen" -> betaald, not "gebetaald").

Most verbs take hebben as their auxiliary. __A smaller set — mostly verbs of motion or change of state, like gaan (to go), komen (to come), worden (to become), and zijn itself — take zijn instead__: "Ik ben naar huis gegaan" (I went home), not "Ik heb naar huis gegaan."
...
Past participles (gekund, gemogen, gewild, gemoeten) exist but are rarely used on their own — when a modal verb combines with another verb in the present perfect, __Dutch typically uses a second bare infinitive instead of the past participle__: "Ik heb dat niet kunnen doen" (I couldn't do that), not "Ik heb dat niet gekund doen."`,
    examples: [
...
__A few diminutives are irregular and just need to be learned as-is__: "schip" (ship) -> "scheepje", not "schipje"; "glas" (glass) -> "glaasje". A couple of words — like "meisje" (girl) — only exist in diminutive form.`,
    examples: [
...
This is the key difference from **subordinating conjunctions** like omdat (because), als (if), or dat (that), which __do change the word order of the clause that follows them, pushing the verb all the way to the end__ — that's a topic for later. For now, these five coordinating conjunctions are the safe, simple way to link ideas.`,
    examples: [
...
In front of a het-word, it depends on whether the noun is **definite** or **indefinite**. With a definite het-word (using het, dit, or dat), add -e: "het grote huis" (the big house). With an indefinite singular het-word — after een, geen, or no article at all, and also after elk, ieder, menig, veel, welk, or zo'n — __don't add -e__: "een groot huis" (a big house), "groot huis" with no article at all, "warm water" (warm water), "vers brood" (fresh bread).

__A few adjectives never take -e in any position__: those already ending in -en (mostly materials, like houten "wooden" or gouden "golden") and the two words linker (left) and rechter (right).

One more small pattern: __after iets (something), niets (nothing), veel (much), weinig (little), or wat (something), an adjective takes -s instead of -e__: "iets moois" (something beautiful), "niets nieuws" (nothing new).`,
    examples: [
